...
const products = list_products.map(p => ({
  ...p,
  price: parsePrice(p.price),
  promo: p.promo ? { name: p.promo.name, value: parsePromoValue(p.promo.value) } : null
}));
...
// Thêm dữ liệu mới
  ...
  if (typeof str === 'string') return Number(str.replace(/\./g, ''));
  return str;
}

function parsePromoValue(val) {
